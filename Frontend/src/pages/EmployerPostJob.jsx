...
const EmployerPostJob = ({ onSubmit, initialData }) => {
  ...
  useEffect(() => {
  if (initialData) {
    // "Làm phẳng" (flatten) dữ liệu từ API để khớp với state của form
    const flattenedData = {
      id: initialData._id, // QUAN TRỌNG: Ánh xạ _id sang id
      title: initialData.title || "",
      position: initialData.position || "",
      location: initialData.location || "",
      detailedAddress: initialData.detailedAddress || "", // Giả sử API trả về cái này

      // QUAN TRỌNG: Lấy từ trong 'salary'
      ...
      experience: String(initialData.experience.value ?? ""), // Chuyển số sang chuỗi
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const formattedData = {
        ...
        experience: {
            value: Number(form.experience), // <-- SỬA Ở ĐÂY
            unit: 'years'
        },
        
        // Sửa 'salary' thành một Object
        salary: {
      ...
      // Xóa các trường không cần thiết mà Backend không nhận
      delete formattedData.minSalary;
      delete formattedData.maxSalary;
      delete formattedData.currency;
      // ---------------------------------

      // BƯỚC 2: Gửi dữ liệu đã định dạng lên Homepage
