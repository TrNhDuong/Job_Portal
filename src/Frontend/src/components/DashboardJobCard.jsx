...
// HÀM FORMAT LƯƠNG – giống bên JobDetailPanel
...
export default function DashboardJobCard({ job, status, onRemove }) {
  const placeholderLogo = `https://ui-avatars.com/api/?name=${
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          <Link
            to={`/jobs/${job._id}`}
            className="font-bold text-lg text-gray-900 hover:text-blue-700"
          >
            {job.title}
          </Link>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 mt-2">
            ...
            <div className="flex items-center gap-1.5">
              <CalendarDays className="w-4 h-4" /> {createdAtLabel}
            </div>
          </div>
        </div>
      </div>
